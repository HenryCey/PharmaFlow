...
**Architecture note:** one new app this sprint, `apps/reports`, matching the Build Request's requested structure (`apps/reports/services/{dashboard,inventory_report,sales_report,purchase_report,financial_report}_service.py`). It owns no data of its own — every figure is computed on demand from `sales`, `purchases`, `stock`, `inventory`, `customers` and `suppliers` — and carries a single permission-only model (`ReportAccess`, `managed = False`, no real table) so `view_inventory_reports` / `view_sales_reports` / `view_purchase_reports` / `view_financial_reports` exist as real Django permissions ahead of the report pages that will enforce them.

### Added

- **`apps/reports/services/`** — the reporting engine: `dashboard_service` (composes the four below into permission-scoped KPI cards, recent activity, and chart data), `inventory_report_service` (current inventory, valuation, low/out-of-stock, near-expiry, batch, movement, stock-adjustment), `sales_report_service` (summary, trend by period, by-drug/customer/cashier, top-selling, slow-moving, receipt history), `purchase_report_service` (summary, by-supplier/drug/date, received/cancelled/outstanding-draft), `financial_report_service` (revenue, purchase cost, estimated gross profit, inventory value, average daily sales). Every report-category function reuses the layer below it rather than re-querying — e.g. `financial_report_service` never queries `Sale`/`PurchaseOrder` directly, only the other three services.
- **Dashboard redesign**, fully permission-aware. `DashboardHomeView` now calls `DashboardService.dashboard_context()` instead of the Sprint 1 placeholder. Real KPI cards (Today's Sales, Today's Purchases, Inventory Value, Estimated Gross Profit, Total Customers, Total Suppliers, Low Stock, Near Expiry, with an Out-of-Stock banner when relevant), Recent Sales/Purchases/Inventory-Movements feeds, and four charts (Sales Trend, Purchase Trend, Top Selling Drugs, Inventory Distribution by Category) — each computed and rendered only for a user who actually holds the relevant permission (`sales.view_sale`, `purchases.view_purchaseorder`, `inventory.change_drug`, `customers.view_customer`, `suppliers.view_supplier`, `stock.view_inventorymovement`, `reports.view_financial_reports`). A widget's underlying query is never run if the user can't see it — no premature caching, just skipping the work.
- **Permission-aware sidebar**: nav items for modules the current user has no permission to access are now omitted entirely rather than shown as a permanently disabled "Coming soon" placeholder — Inventory/Sales/Purchases/Customers/Suppliers are real, built modules as of Sprint 4, so a greyed-out label for one of them read as broken, not "coming soon." Reports remains the one genuine placeholder (no report pages exist yet), now gated to only appear for users holding at least one `reports.*` permission.
- **Empty-state messaging**: Near Expiry, Today's Sales, Today's Purchases, Recent Sales, and Recent Purchases each show friendly copy ("No drugs expiring within 30 days.", "No sales recorded today.", etc.) instead of a blank or zeroed-looking card when there's genuinely nothing to show.
- **Chart.js 4.5.1**, vendored locally into `static/vendor/` (no CDN, consistent with the Sprint 4 offline-first policy) — added alongside the existing HTMX/Alpine vendor files.
- **`{% block extra_js %}`** added to `templates/layout/base.html` — the first page-specific script hook in the shell (purely additive; every other page renders identically).
- **`apps/reports/tests/test_services.py`** — 14 tests against real seeded data (not mocks): inventory valuation, low/near-expiry stock, sales-by-drug, cashier row-level scoping, walk-in grouping, purchases-by-date, full `dashboard_context()` for Owner/Pharmacist/Cashier, the `change_drug`-vs-`view_drug` permission distinction, and full-page rendering through the real URL/view/template for all three roles. `pytest.ini` added (none existed before this release, despite `pytest`/`pytest-django` already being in `requirements.txt` since Sprint 1).
- `seed_role_permissions` extended: Administrator gets all four reports permissions; Pharmacist gets Inventory + Sales reports (mirroring what they already manage day to day); Cashier gets none. **Flagged for confirmation** — no Sprint 5 Build Request specified a reports permissions matrix, same caveat as the Sprint 4 Pharmacist/Purchases note.

### Fixed

Issues found and corrected during this sprint's two test-build/QA rounds, before this version was tagged:

- **`sales_by_drug` raised `FieldError` under real data.** Annotating `quantity` before `revenue` in the same `.annotate()` call made Django resolve the inner `F("quantity")` used to compute `revenue` against the newly-added `quantity` *annotation* (itself a `Sum`, i.e. an aggregate) instead of `SaleItem`'s real field, raising `"... is an aggregate"`. Fixed by annotating `revenue` first; documented in-line since the failure mode is non-obvious and easy to reintroduce.
- **`purchases_by_date(period="daily")` raised `OperationalError`** (reproduced against SQLite during verification; `purchase_date` is a plain `DateField`, so truncating it to day-precision was both wrong and backend-fragile). Daily grouping now uses the field directly; `TruncWeek`/`Month`/`Year` are still used for the genuinely-aggregating periods.
- **Dashboard's out-of-stock banner linked nowhere useful.** Referenced `inventory:drug-list` (hyphen; the real name is `drug_list`, underscore) and a `?stock=out_of_stock` querystring filter that `DrugListView` doesn't implement — would have silently shown the unfiltered list. Fixed to link to the real drug list URL.
- **Broken currency formatting on the dashboard.** The KPI cards' first draft piped a currency symbol string through the `add` filter with a `Decimal` argument; Django's `add` filter returns `""` when it can't parse both sides as numbers, so every money figure would have rendered blank. Fixed using `components/_card.html`'s own documented pattern ("for arbitrary HTML body content, wrap manually using the same classes") instead of trying to compose two values through a single `include`.
- **`inventory.view_drug` was too coarse a permission for inventory-value cards.** Cashier holds `view_drug` for POS drug lookup only, which would have surfaced stock valuation/health figures (Inventory Value, Low Stock, Near Expiry) to a role that should only see individual drug names and prices at the till. Fixed to gate those specific cards on `inventory.change_drug` instead, which only Owner/Administrator/Pharmacist hold.
- **`requirements.txt`: `pytest>=8.0` with no upper bound resolves to pytest 9.1.1, which is incompatible with `pytest-django`** (`AttributeError: type object 'PytestDjangoTestCase' has no attribute '_pre_setup_ran_eagerly'` — reproduced while adding this sprint's test suite). Pinned to `pytest>=8.0,<9.0` / `pytest-django>=4.8,<5.0`.

### Known Limitations

- **No report pages yet.** Inventory/Sales/Purchase/Financial report list views, filters, and CSV/Excel/PDF export do not exist — the Reports sidebar entry stays a "Coming soon" placeholder. The service layer backing them is complete and tested; this is the next Sprint 5 increment.
- **Near Expiry / Batch Report are per-drug, not per-unit.** PharmaFlow has no FEFO/batch-level stock ledger (`FUTURE_ENHANCEMENTS.md` PFE-002 is exactly this, deferred to v2.x+) — expiry is tracked per received Purchase Item, not per remaining unit. `near_expiry_drugs()` surfaces any drug with stock on hand *and* at least one batch expiring soon; it cannot guarantee the specific units on the shelf are that batch.
- **"Estimated" Gross Profit is exactly that.** `SaleItem` snapshots only the selling price, never a cost; the documented costing policy is Last Cost Wins, not FIFO/weighted-average (`FUTURE_ENHANCEMENTS.md` PFE-010). Cost of goods sold is approximated as quantity-sold × each drug's *current* cost price, which will misstate historical periods where a drug's cost has since changed.
- **Pharmacist's dashboard shows more than the Sprint 5 refinement brief's illustrative example.** Per Pharmacist's existing (unmodified this sprint) Sprint 1–4 permission grant, they hold full `sales.view_sale`/`view_all_sales` and `suppliers.view_supplier`, so Sales-related widgets and Total Suppliers appear on their dashboard — a permission-matrix question for a future round, not a bug in this sprint's gating logic.

### Upgrade Notes

```bash
pip install -r requirements.txt   # pytest/pytest-django version pins changed
python manage.py migrate          # applies reports.0001_initial — no real table (ReportAccess is managed=False), only registers permissions
python manage.py seed_role_permissions
```

No destructive schema changes. No existing Sprint 1–4 table, view, or template was altered beyond `templates/layout/base.html`'s additive `{% block extra_js %}` and the Sprint 1 placeholder dashboard (`apps/dashboard/views.py` / `templates/dashboard/home.html`), which this sprint explicitly replaces.
